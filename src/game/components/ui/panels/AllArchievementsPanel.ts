...
const GamesICo = ["Psicopata", "Muerte", "Tragon", "Minijuego", "Platino"];

const GameName = ["Psicopata", "Muerte", "Tragon", "Minijuego", "Platino"];
const GameDescription = [
    "Estas demente por hacerle eso a tu foca",
    "Porque la dejaste morir?????",

    "Eso engordala para luego comerla",
    "Juega un minijuego con tu foca",
    "Enserio platinaste un juego de una foca?",
];

export class AchievementsAllPanel extends Sizer {
    private Hitbox: Phaser.GameObjects.Rectangle;
    private archivements: [[string, Sizer]] = [];
    constructor(scene: Phaser.Scene) {
        ...
        for (let i = 0; i < GamesICo.length; i++) {
            ...
            if (localStorage.getItem(GameName[i]) == "true") {
                GameSizer.setAlpha(1);
            } else {
                GameSizer.setAlpha(0.5);
            }

            this.archivements.push([GameName[i], GameSizer]);
    ...
    Start() {
        ...
        this.archivements.forEach(([name, sizer]) => {
